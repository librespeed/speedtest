...
$id = $_GET["id"];
include_once('../telemetry/telemetryEntry.php');
require '../telemetry/idObfuscation.php';

if($enable_id_obfuscation) $id=deobfuscateId($id);
$entry = $repository->find($id);
$ispinfo=json_decode($entry->ispinfo, true)["processedString"];
$dash=strpos($ispinfo,"-");
if(!($dash===FALSE)){
...
$dlMeterBbox=imageftbbox($FONT_2_SIZE,0,$FONT_2,$entry->downloadspeed);
$ulMeterBbox=imageftbbox($FONT_2_SIZE,0,$FONT_2,$entry->uploadspeed);
$pingMeterBbox=imageftbbox($FONT_2_SIZE,0,$FONT_2,$entry->ping);
$jitMeterBbox=imageftbbox($FONT_2_SIZE,0,$FONT_2,$entry->jitter);
...
imagefttext($im,$FONT_2_SIZE,0,$POSITION_X_DL-$dlMeterBbox[4]/2,$POSITION_Y_2,$TEXT_COLOR_2,$FONT_2,$entry->downloadspeed);
imagefttext($im,$FONT_2_SIZE,0,$POSITION_X_UL-$ulMeterBbox[4]/2,$POSITION_Y_2,$TEXT_COLOR_2,$FONT_2,$entry->uploadspeed);
imagefttext($im,$FONT_2_SIZE,0,$POSITION_X_PING-$pingMeterBbox[4]/2,$POSITION_Y_2,$TEXT_COLOR_2,$FONT_2,$entry->ping);
imagefttext($im,$FONT_2_SIZE,0,$POSITION_X_JIT-$jitMeterBbox[4]/2,$POSITION_Y_2,$TEXT_COLOR_2,$FONT_2,$entry->jitter);
